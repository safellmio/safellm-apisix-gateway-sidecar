...
def test_gateway_health():
    ...
    try:
        ...
        pytest.skip(f"Gateway health check failed: {exc}")

    if result.returncode != 0:
        pytest.skip(f"Gateway health check failed (curl rc={result.returncode})")

    status = result.stdout.strip()
    if status != '200':
        pytest.skip(f"Gateway not running (status={status})")
...
def load_false_positive_dataset() -> List[Tuple[str, str, str]]:
    ...
    """
    datasets = [
        "GateWayApsix/safellm/tests/datasets/edge_cases_false_positives.csv",
        "GateWayApsix/safellm/tests/datasets/known_false_positives.csv",
        "GateWayApsix/safellm/tests/datasets/latency_safe_prompts.csv"
    ]
